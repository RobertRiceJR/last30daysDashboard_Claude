...
def get_x_source_with_method(config: dict[str, Any]) -> tuple[str | None, str]:
    ...
    if config.get("AUTH_TOKEN") and config.get("CT0"):
        ...
        return "bird", method
    # Keyless fork: xurl / Bird X backends removed.
    return None, "none"
...
def get_x_source(config: dict[str, Any]) -> str | None:
    ...
    """
    # Keyless fork: all X/Twitter backends (xAI / Bird / xurl) were removed.
    return None
...
def get_x_source_status(config: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    # Keyless fork: X/Twitter backends (Bird / xAI / xurl) removed.
    return {
        "source": None,
